# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .agent_product_details_response_package_dimensions_size_depth import (
    AgentProductDetailsResponsePackageDimensionsSizeDepth,
)
from .agent_product_details_response_package_dimensions_size_length import (
    AgentProductDetailsResponsePackageDimensionsSizeLength,
)
from .agent_product_details_response_package_dimensions_size_width import (
    AgentProductDetailsResponsePackageDimensionsSizeWidth,
)


class AgentProductDetailsResponsePackageDimensionsSize(UniversalBaseModel):
    width: typing.Optional[AgentProductDetailsResponsePackageDimensionsSizeWidth] = None
    depth: typing.Optional[AgentProductDetailsResponsePackageDimensionsSizeDepth] = None
    length: typing.Optional[AgentProductDetailsResponsePackageDimensionsSizeLength] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
